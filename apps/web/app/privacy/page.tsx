...
export default function PrivacyPage() {
  return <main className="study-page legal-page">
    ...
    <article className="legal-card">
      ...
      <section>
        <h2>4. 録画、フレーム、公開StudyとAI連携</h2>
        <p>アップロードされた元動画は、フレーム生成処理のため一時的に保存され、正常完了または最終失敗後に削除する設計です。生成されたフレームやStudyデータは、選択した公開範囲とサービスの保持設定に従って保存されます。</p>
        <p>公開設定のStudyは、URLを知る第三者のほか、ChatGPT、Codexその他のMCP対応AIクライアントから閲覧される可能性があります。公開MCPは、Study ID、処理状態、フレーム数、ランク、DPI、ゲーム内感度、映像設定、利用者が入力した状況メモ、Study作成時に保存されたコーチングプロンプト、フレーム画像およびそれらの公開URLを、コーチングのための読み取り専用データとして返す場合があります。</p>
        <p>非公開Studyは認証された所有者向けに提供し、公開MCPでは所有者セッションを利用せず、private Studyの内容を返しません。</p>
      </section>
      ...
      <section>
        <h2>6. 第三者提供</h2>
        <p>法令に基づく場合、本人の同意がある場合、または本サービスの提供に必要な業務委託先へ必要最小限の情報を取り扱わせる場合を除き、個人情報を第三者へ不当に提供しません。利用者がStudyを公開設定にした場合に、その公開範囲に従って第三者またはAIクライアントへStudyデータが提供されることを含みます。</p>
      </section>
      ...
      <section>
        ...
        <p>利用者は、自身の情報について削除、訂正その他の問い合わせを行うことができます。依頼方法と安全上の注意は<Link href="/support">サポートページ</Link>を確認してください。</p>
      </section>
