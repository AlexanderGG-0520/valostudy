...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  return handle(async () => {
    ...
    const { partNumber } = z.object({ partNumber: z.number().int().positive().max(256) }).parse(await jsonBody(request));
    ...
    let size: number;
    try { size = partSize(upload.expectedBytes, partNumber); }
    catch { throw new HttpError(400, "Part out of range"); }
    const url = await new Storage().partUrl(upload.objectKey, upload.uploadId, partNumber, size, Math.min(900, remaining));
