...
export async function processVideo(job: Pick<Job, "data" | "id" | "attemptsMade" | "opts">) {
  ...
  try {
    await db().transaction(async (tx) => {
      ...
      await tx.update(jobs).set({ status: "processing", attempts: job.attemptsMade + 1, error: null, updatedAt: new Date() }).where(eq(jobs.studyId, id));
    });
    ...
    await pipeline(Readable.from(chunks()), limit, createWriteStream(source), { signal: AbortSignal.timeout(300000) });
    ...
    log("frames_extracted", { studyId: id, jobId: job.id, stage, duration: Date.now() - ffmpegStarted, frameCount: result.frames.length });
    stage = "persist";
    ...
    await db().transaction(async (tx) => {
      ...
      await tx.update(jobs).set({ status: "completed", error: null, updatedAt: new Date() }).where(eq(jobs.studyId, id));
    });
    log("processing_completed", { studyId: id, jobId: job.id, stage, duration: Date.now() - started, frameCount: rows.length });
  } catch (e) {
    const status = job.attemptsMade + 1 < (job.opts.attempts ?? 1) ? "queued" : "failed";
    await db().transaction(async (tx) => {
      await tx.update(studies).set({ status }).where(and(eq(studies.id, id), inArray(studies.status, ["queued", "processing", "failed"])));
      await tx.update(jobs).set({ status, error: "Video processing failed; see worker logs", updatedAt: new Date() })
        .where(and(eq(jobs.studyId, id), inArray(jobs.status, ["pending", "queued", "processing", "failed"])));
    });
    log("processing_failed", { studyId: id, jobId: job.id, stage, duration: Date.now() - started,
