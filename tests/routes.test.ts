...
it("renders a public AI entrypoint with player settings, prompt, and machine-readable links", async () => {
  ...
  expect(mocks.buildManifest).toHaveBeenCalledWith(id);
  expect(html).not.toContain("<script>alert(1)</script>");
  expect(html).toContain("&lt;script&gt;");
});
...
it("paginates AI frame indexes and links directly to frame evidence", async () => {
  const frames = Array.from({ length: 241 }, (_, index) => ({
    timestampMs: index * 200,
    url: `/${id}/frames/${String(index + 1).padStart(6, "0")}.jpg`,
  }));
  mocks.buildManifest.mockResolvedValue({ ...manifest, frames });
