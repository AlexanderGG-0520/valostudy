...
export class Storage {
  ...
  constructor() {
    ...
    this.client = new S3Client({ endpoint: c.S3_ENDPOINT, region: c.S3_REGION, forcePathStyle: true,
  ...
  abort(key: string, uploadId: string) {
    return this.client.send(new AbortMultipartUploadCommand({ Bucket: this.bucket, Key: key, UploadId: uploadId }));
  }
  async head(key: string) {
    try { return await this.client.send(new HeadObjectCommand({ Bucket: this.bucket, Key: key })); }
    catch (e) { if (e instanceof Error && e.name === "NotFound") return null; throw e; }
  }
  get(key: string) { return this.client.send(new GetObjectCommand({ Bucket: this.bucket, Key: key })); }
  putFrame(key: string, bytes: Uint8Array) {
    return this.client.send(new PutObjectCommand({ Bucket: this.bucket, Key: key, Body: bytes, ContentType: "image/webp" }));
  }
  delete(key: string) { return this.client.send(new DeleteObjectCommand({ Bucket: this.bucket, Key: key })); }
}
